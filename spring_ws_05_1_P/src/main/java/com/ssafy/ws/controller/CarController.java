...
public class CarController {
	 ...
	 */
	 @PostMapping("/regist")
	 public String doRegist(@ModelAttribute Car car, @RequestPart MultipartFile file) throws IllegalStateException, IOException {
	 	if (!file.isEmpty()) {
	 		// 파일을 저장할 폴더 지정
			...
			// res.getFile() : res 객체의 getFile 메서드를 호출하여 파일을 저장할 디렉토리의 경로를 가져옴, 이 경로는 디렉토리 객체로 변환됨
			// car.getImg() : car 객체의 getImg 메서드를 호출하여 저장할 파일의 이름을 가져옴 
			// file의 내용을 해당 디렉토리와 파일 이름으로 저장 
